...
from kb.bert_tokenizer_and_candidate_generator import sep_token
from kb.bert_tokenizer_and_candidate_generator import start_token
...
def replace_candidates_with_random_entity(candidates, spans_to_random):
    for candidate_key in candidates.keys():
        ...
        all_entities = list(set(chain.from_iterable(
            candidates[candidate_key]['candidate_entities'])))
        ...
        for ind in indices_to_random:
            ...
            candidates[candidate_key]['candidate_entities'][ind] = list(
                rand_entities)
...
class BertTokenizerCandidateGeneratorMasker:
    ...
    def __init__(self,
        ...
        self._label_indexer = {
            "lm_labels": foo._bert_single_id_indexer["tokens"]}
    ...
    def tokenize_candidates_mask(self, sentence1: str, sentence2: str):
        ...
        fields['lm_label_ids'] = TextField(
            [Token(t, text_id=self.tokenizer_and_candidate_generator.bert_tokenizer.vocab[t])
             for t in lm_labels],
...
class BertPreTrainingReader(DatasetReader):
    """
    This DatasetReader is designed to read in text corpora segmented into
    sentences and yield documents for training BERT models.
    ...
    next_sentence_label : ``LabelField``
        Next sentence label: is the second sentence the next sentence
        following the first one, or is it a randomly selected sentence.
    ...
    def text_to_instance(self,
        ...
        fields = self._tokenizer_masker.tokenize_candidates_mask(
            sentence1, sentence2)
...
@DatasetReader.register("bert_pre_training_pregen_cand")
class BertPreTrainingPreGenCandidatesReader(DatasetReader):
    """
    This DatasetReader is designed to read in text corpora segmented into
    sentences and yield documents for training BERT models.
    ...
    def __init__(self,
        ...
        self._tokenizer_masker = BertTokenizerPregenCandidateMasker(
            tokenizer_and_candidate_generator,
            max_predictions_per_seq=max_predictions_per_seq,
            masked_lm_prob=masked_lm_prob,
            mask_candidate_strategy=mask_candidate_strategy)
        self.cand_dir = candidate_directory

    @overrides
    def _read(self, file_path: str):
        """ make sure file has a corresponding file named identically but
            with a .cand extension to get candidates from.
        """
        warn_once_flag = True
        # if `file_path` is a URL, redirect to the cache
        with open(cached_path(file_path), 'r') as fin:
        ...
        # removing extension
        candidate_file_path = path.splitext(file_path)[0]
        # separating base filename from path
        # dirname, basename = path.split(candidate_file_path)
        _, basename = path.split(candidate_file_path)
        # adding candidate directory name in path
        candidate_file_path = path.join(self.cand_dir, basename)
        #                     path.join(dirname, "cand_files", basename)
        # adding proper candidate file extension
        candidate_file_path += '.cand'

        with open(candidate_file_path, 'rb') as candfile:
            candidate_lookup = pickle.load(candfile)
        for i, line in enumerate(lines):
            label, sentence1, sentence2 = line.strip().split('\t')
            try:
                cand_seq1 = candidate_lookup[
                    md5(sentence1.encode()).hexdigest()]
                cand_seq2 = candidate_lookup[
                    md5(sentence2.encode()).hexdigest()]
            except KeyError:
                if warn_once_flag:
                    logger.warning(f'Found missing candidates for {file_path}')
            yield self.text_to_instance(sentence1, sentence2, int(label),
                                        cand_seq1, cand_seq2)
    ...
    def text_to_instance(self,
                         ...
                         sentence2: str,
                         next_sentence_label: int,
                         cand_seq1: Dict[str, List[Any]],
                         cand_seq2: Dict[str, List[Any]]):

        fields = self._tokenizer_masker.tokenize_candidates_mask(
            sentence1, sentence2, cand_seq1, cand_seq2)
...
class BertTokenizerPregenCandidateMasker(
        BertTokenizerCandidateGeneratorMasker):
