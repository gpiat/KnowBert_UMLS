...
def prior_entity_candidates(candidates_file: str,
    ...
    allowed_entities_set: restrict the candidate entities to only this set.
    for example the most frequent 1M entities. First this restiction applies
     and then the cand_ent_num.
    """
    ...
    # both higher absolute frequency and longer cand list
    clear_conflict_winner = 0
    ...
    with open(candidates_file) as fin:
        ...
        for i, line in enumerate(fin):
            ...
            for e in entities:
                ...
                if allowed_entities_set is not None and\
                   name not in allowed_entities_set:
                    pass
                else:
            ...
            if entity_candidates:
                if mention in p_e_m:
                    ...
                    # for each mention we have a list of tuples
                    # (ent_id, name, score)
                    p_e_m[mention] = entity_candidates
                    mention_total_freq[mention] = absolute_freq

    print("duplicate_mentions_cnt: ", duplicate_mentions_cnt)
    print("end of p_e_m reading. wall time:",
          (time.time() - wall_start) / 60, " minutes")
    print("p_e_m_errors: ", p_e_m_errors)
    print("incompatible_ent_ids: ", incompatible_ent_ids)

    wall_start = time.time()
    # two different p(e|m) mentions can be the same after lower() so we merge
    # the two candidate entities lists. But the two lists can have the same
    # candidate entity with different score we keep the highest score. For
    # example if "Obama" mention gives 0.9 to entity Obama and OBAMA gives
    # 0.7 then we keep the 0.9 . Also we keep as before only the cand_ent_num
    # entities with the highest score
...
STOP_SYMBOLS = set().union(LIST_PUNCT, LIST_ELLIPSES,
                           LIST_QUOTES, LIST_CURRENCY)


def span_filter_func(span: List[str]):
    """
    This function halves the number of suggested mention spans
    whilst not affecting gold span recall at all. It can probably
    be improved further.
    """
...
class WikiCandidateMentionGenerator(MentionGenerator):
    ...
    defaults = {
        "candidates_file": ("https://allennlp.s3-us-west-2."
                            "amazonaws.com/knowbert/wiki_entity_linking/"
                            "prob_yago_crosswikis_wikipedia_p_e_m.txt"),
        "wiki_file": ("https://allennlp.s3-us-west-2."
                      "amazonaws.com/knowbert/wiki_entity_linking/"
                      "wiki_name_id_map.txt"),
        "redirections_file": ("https://allennlp.s3-us-west-2."
                              "amazonaws.com/knowbert/wiki_entity_linking/"
                              "wiki_redirects.txt"),
        "disambiguations_file": ("https://allennlp.s3-us-west-2."
                                 "amazonaws.com/knowbert/wiki_entity_linking/"
                                 "wiki_disambiguation_pages.txt"),
        "entity_world_path": ("https://allennlp.s3-us-west-2."
                              "amazonaws.com/knowbert/wiki_entity_linking/"
                              "wiki_id_to_string.json"),
    }
    ...
    def __init__(self, candidates_file: str = None,
        ...
        if pickle_cache_file is not None:
            ...
            self.p_e_m, self.p_e_m_low, self.mention_total_freq =\
    ...
    def get_mentions_with_gold(self, text: str, gold_spans, gold_entities,
    ...
    def process(self, span: Union[List[str], str], lower=False) ->\
        ...
        (entity_id, entity_candidate,
         p(entity_candidate | mention string)) pairs.
        """
        if self.random_candidates:
        ...
        # Makes all first chars of words uppercase,
        # eg barack obama -> Barack Obama.
        title = span.title()
